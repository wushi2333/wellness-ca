...
public final class WebViewModels {
    ...
    static DashboardSummary dashboard(List<DailyWellnessResponse> dailies, String tip) {
    ...
    /** True if the displayed week is the latest (current) week → next button should be disabled. */
    static boolean isLatestWeek(LocalDate monday) {
        return !monday.isBefore(currentWeekMonday());
    }

    /** True if there is at least one sleep record dated before the displayed week's Monday. */
    static boolean hasSleepBefore(List<DailyWellnessResponse> dailies, LocalDate monday) {
        for (DailyWellnessResponse d : dailies) {
            if (parseDate(d.recordDate).isBefore(monday) && d.sleep != null) return true;
        }
        return false;
    }

    /** True if there is at least one exercise record dated before the displayed week's Monday. */
    static boolean hasExerciseBefore(List<DailyWellnessResponse> dailies, LocalDate monday) {
        for (DailyWellnessResponse d : dailies) {
            if (parseDate(d.recordDate).isBefore(monday) && d.exercises != null && !d.exercises.isEmpty()) return true;
        }
        return false;
    }
    ...
    private static List<String> dayLabels(LocalDate monday) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            labels.add(date.getDayOfWeek().getDisplayName(TextStyle.SHORT, locale));
        }
        return labels;
    }
